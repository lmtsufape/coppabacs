...
public class CorService implements CorServiceInterface {
	...
	public Cor findCorById(long id) {
		return repository.findById(id).orElseThrow( () -> new RuntimeException("It doesn't exist Cor with id = " + id));
	}
	...
	public void deleteCor(long id){
		Cor obj = repository.findById(id).orElseThrow( () -> new RuntimeException("It doesn't exist Cor with id = " + id));
